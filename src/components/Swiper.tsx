...
import  { Navigation, Pagination }
 from 'swiper/modules'; // Import Swiper core and required modules
...
const Carousel = () => {
    ...
    const fetchImages = async () => {
        try {
            ...
            const imageUrls = filteredImages.map(image => image.urls.regular);
            setImages(imageUrls);
        } catch (error) {
  ...
  key={index}
>
  <img src={imageUrl} alt={`Slide ${index + 1}`} className="swiper-image" />
</SwiperSlide>
      ))}
    </Swiper>
  );
